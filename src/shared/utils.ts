import { EMBED_TO, EMBED_TO_1, IMAGE_URL } from "./constants";
...
export const embedMovie = (id: number): string => `${EMBED_TO}/movie?tmdb=${id}`;
export const embedMovie1 = (id : number): string => `${EMBED_TO_1}/movie/${id}`;

export const embedTV = (id: number, season: number, episode: number): string =>
  `${EMBED_TO}/tv?tmdb=${id}&s=${season}&e=${episode}`;

export const embedTV1 = (id: number, season: number, episode: number): string =>
  `${EMBED_TO_1}/tv/${id}/${season}/${episode}`;
