...
const excludedLanguages = ['zh', 'ko', 'hi', 'in', 'jp', 'ml'];
    ...
    switch (index) {
      ...
      case 3:
        final.similar = current.data.results
          .filter((item: Items) => !excludedLanguages.includes(item.original_language)) // Filtra os filmes com idiomas excluídos
          .map((item: Items) => ({ ...item, media_type: "movie" }));
        break;
